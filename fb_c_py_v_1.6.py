    ...
    if cam_tye=='1':
        ...
        if cre_frmat=='1':
            ...
            for f_i in range(math.ceil(len(files)/row_num)):
                new_data.loc[f_i*row_num:(f_i+1)*row_num,'Campaign Name'] = str(raw_data_backup['Campaign Name'][0])+'_num_'+str(f_i+1)
            ...
            # campaign命名 以row_num
            for f_i in range(math.ceil(len(files)/row_num)):
                #new_data['Campaign Name'][f_i*row_num:(f_i+1)*row_num] = str(raw_data_backup['Campaign Name'][0])+'_num_'+str(f_i+1)
                new_data.loc[f_i*row_num:(f_i+1)*row_num,'Campaign Name'] = str(raw_data_backup['Campaign Name'][0])+'_num_'+str(f_i+1)
            # 一个广告组一个广告的时候，广告组赋值=广告name 其他情况，使用原
            if ad_num==adset_num:
                new_data['Ad Set Name']=new_data['Ad Set Name']+'_'+new_data['Ad Name']
